...
class PanelHandler(BaseHTTPRequestHandler):
    ...
    def api_tunnels(self) -> None:
        ...
        self._send_json({"ok": True, "tunnels": tunnels, "defaults": defaults})
    ...
    def api_create_tunnel(self, data: Dict[str, Any]) -> None:
        ...
        cfg = manager.create_mesh_profile(
            ...
            remote_forward_ip=(str(data.get("remote_forward_ip") or "").strip() or None),
        )
        if data.get("forwarded_ports"):
            cfg.forwarded_ports = parse_ports(str(data.get("forwarded_ports")))
        started = False
        message = "Tunnel ذخیره شد"
        if data.get("autostart", True):
            started, message = EasyTierManager(cfg).start_tunnel()
        if get_forward_mode() == "haproxy" and cfg.forwarded_ports:
            HAProxyManager().apply()
        self._send_json({"ok": True, "started": started, "message": message, "tunnel": tunnel_to_payload(cfg.name)})
    ...
    <div id="app" class="layout hide">
      ...
      <main>
        <section id="dashboard" class="page active card">
          ...
          <p class="muted">اینجا فقط وضعیت‌های اصلی نمایش داده می‌شود. خطای نصب EasyTier و وضعیت HAProxy از همین بخش قابل تشخیص است.</p>
          <div id="statusCards" class="grid"></div>
          <div id="easyInstallBox" style="margin-top:14px"></div>
        </section>

        <section id="wizard" class="page card">
          <h2>ساخت تونل مرحله‌ای</h2>
          <div class="steps"><span id="s1" class="step on">سناریو</span><span id="s2" class="step">تنظیمات شبکه</span><span id="s3" class="step">Peerها</span><span id="s4" class="step">Forward و اجرا</span></div>

          <div id="w1">
            <h3>مرحله ۱: نقش این سرور چیست؟</h3>
            <div class="grid2">
              <div class="hint"><b>خارج / Hub</b><p>برای سرور خارج که ایران‌ها به آن وصل می‌شوند. Peer می‌تواند خالی باشد.</p><button onclick="pickScenario('hub')">انتخاب Hub</button></div>
              <div class="hint"><b>ایران / Node</b><p>برای سرور ایران. در مرحله Peer باید IP سرور خارج را وارد کنید.</p><button onclick="pickScenario('iran')">انتخاب Node ایران</button></div>
            </div>
          </div>

          <div id="w2" class="hide">
            <h3>مرحله ۲: تنظیمات شبکه</h3>
            <div class="row3">
              <div><label>نام تونل</label><input id="name" placeholder="iran1 یا kharej-hub" /></div>
              <div><label>نقش</label><select id="role"><option value="hub">خارج / Hub</option><option value="iran">ایران / Node</option><option value="node">Node معمولی</option></select></div>
              <div><label>Secret شبکه</label><input id="secret" value="vortexl2" /></div>
            </div>
            <div class="row3" style="margin-top:12px">
              <div><label>IP داخلی EasyTier</label><input id="local_ip" dir="ltr" placeholder="10.155.155.1" /></div>
          ...
          <div id="w3" class="hide">
            <h3>مرحله ۳: Peerها</h3>
            <p class="muted">هر Peer را در یک خط وارد کنید. مثال: <span dir="ltr">1.2.3.4:2070 # kharej</span></p>
            <textarea id="peers" dir="ltr" placeholder="KHAREJ_PUBLIC_IP:2070 # kharej"></textarea>
            <div id="peerHint" class="hint" style="margin-top:12px"></div>
          ...
          <div id="w4" class="hide">
            <h3>مرحله ۴: Forward و اجرا</h3>
            <div class="row">
              <div><label>Remote Forward IP</label><input id="remote_forward_ip" dir="ltr" placeholder="مثلاً 10.155.155.2" /></div>
              <div><label>Forwarded ports</label><input id="forwarded_ports" dir="ltr" placeholder="80,443,2087" /></div>
            </div>
            <p class="muted">اگر فعلاً Forward لازم ندارید، این دو فیلد را خالی بگذارید. HAProxy بدون پورت دیگر خطا نمی‌دهد.</p>
            <div id="review" class="code"></div>
            <div class="btns" style="margin-top:14px"><button class="secondary" onclick="wizardStep(3)">قبلی</button><button class="ok" onclick="createTunnel()">ذخیره و اجرا</button></div>
          </div>
        </section>
        ...
        <section id="forwarding" class="page card">
          ...
          <p class="muted">اول روی یک Tunnel پورت و Remote Forward IP تعریف کنید، سپس HAProxy را فعال کنید. اگر هیچ listener وجود نداشته باشد، HAProxy به‌صورت امن stop می‌ماند و خطا نمی‌دهد.</p>
        ...
        <section id="diagnostics" class="page card">
          ...
          <p class="muted">برای لاگ هر تونل از بخش مدیریت تونل‌ها دکمه «نمایش لاگ» را بزنید. وضعیت نصب EasyTier و HAProxy در همین صفحه هم خلاصه می‌شود.</p>
...
async function api(path, opts={}){opts.headers=Object.assign({'Content-Type':'application/json','X-Vortex-Token':localStorage.vortexToken||''},opts.headers||{});const r=await fetch(path,opts);const j=await r.json().catch(()=>({ok:false,error:'Invalid response'}));if(r.status===401){showLogin();throw new Error('Unauthorized')}return j}
...
async function loadAll(){await Promise.all([loadStatus(),loadTunnels()])}
...
async function loadTunnels(){const j=await api('/api/tunnels');if(!j.ok)return;const d=j.defaults||{};if(!qs('local_ip').value){qs('local_ip').value=d.local_ip||'';qs('port').value=d.port||2070;qs('rpc_port').value=d.rpc_port||15888}const box=qs('tunnels');if(!j.tunnels.length){box.innerHTML='<p class="muted">هنوز تونلی ساخته نشده. از بخش «ساخت تونل مرحله‌ای» شروع کنید.</p>';return}box.innerHTML=j.tunnels.map(t=>`<div class="tunnel"><div class="tunnel-head"><div><h3>${escapeHtml(t.name)} <span class="${t.active?'status-ok':'status-bad'}">● ${escapeHtml(t.status)}</span></h3><div class="muted">${escapeHtml(t.role||'node')} — ${escapeHtml(t.local_ip)} — ${escapeHtml(t.interface_name)} — listen:${t.port} rpc:${t.rpc_port}</div><div style="margin-top:8px">${(t.peers||[]).map(p=>`<span class="pill" dir="ltr">${escapeHtml(p.protocol||'tcp')}://${escapeHtml(p.host)}:${p.port}</span>`).join(' ')||'<span class="pill">Listener only</span>'}</div></div><div class="btns"><button class="ok" onclick="act('${t.name}','start')">Start</button><button class="warn" onclick="act('${t.name}','restart')">Restart</button><button class="secondary" onclick="act('${t.name}','stop')">Stop</button><button class="danger" onclick="delTunnel('${t.name}')">Delete</button></div></div><div class="row" style="margin-top:12px"><div><label>Remote Forward IP</label><input id="rfi_${t.name}" dir="ltr" value="${escapeHtml(t.remote_forward_ip||'')}"/><label style="margin-top:8px">Ports</label><input id="ports_${t.name}" dir="ltr" value="${(t.forwarded_ports||[]).join(',')}"/><button class="secondary" style="margin-top:8px" onclick="savePorts('${t.name}')">ذخیره پورت‌ها</button></div><div><label>Peers</label><textarea id="peers_${t.name}" dir="ltr">${(t.peers||[]).map(p=>`${p.protocol||'tcp'}://${p.host}:${p.port}${p.name?' # '+p.name:''}`).join('\n')}</textarea><button class="secondary" style="margin-top:8px" onclick="savePeers('${t.name}')">ذخیره Peerها و ری‌استارت</button></div></div><details style="margin-top:12px"><summary>Command</summary><div class="code">${escapeHtml(t.command||'')}</div></details><div class="btns" style="margin-top:10px"><button class="secondary" onclick="logs('${t.name}')">نمایش لاگ</button><button class="secondary" onclick="peers('${t.name}')">Peer Stats</button></div><div id="out_${t.name}" class="code" style="display:none;margin-top:10px"></div></div>`).join('')}
function escapeHtml(s){return String(s||'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]))}
function pickScenario(role){qs('role').value=role;if(!qs('name').value)qs('name').value=role==='hub'?'kharej-hub':'iran-node';if(role==='hub'){qs('peerHint').innerHTML='برای Hub می‌توانید Peerها را خالی بگذارید. ایران‌ها با IP عمومی همین سرور به آن وصل می‌شوند.';qs('peers').placeholder='اختیاری: IRAN_PUBLIC_IP:2070 # iran1'}else{qs('peerHint').innerHTML='برای ایران، IP عمومی سرور خارج/Hub را وارد کنید. مثال: <span dir="ltr">1.2.3.4:2070 # kharej</span>';qs('peers').placeholder='KHAREJ_PUBLIC_IP:2070 # kharej'}wizardStep(2)}
...
function reviewWizard(){const data={name:qs('name').value,role:qs('role').value,local_ip:qs('local_ip').value,port:qs('port').value,rpc_port:qs('rpc_port').value,peers:qs('peers').value,remote_forward_ip:qs('remote_forward_ip').value,forwarded_ports:qs('forwarded_ports').value};qs('review').textContent=JSON.stringify(data,null,2)}
async function createTunnel(){reviewWizard();const body={name:qs('name').value,role:qs('role').value,network_secret:qs('secret').value,local_ip:qs('local_ip').value,port:qs('port').value,rpc_port:qs('rpc_port').value,peers:qs('peers').value,remote_forward_ip:qs('remote_forward_ip').value,forwarded_ports:qs('forwarded_ports').value,autostart:true};const j=await api('/api/tunnel/create',{method:'POST',body:JSON.stringify(body)});toast(j.message||j.error,j.ok);if(j.ok){['name','peers','remote_forward_ip','forwarded_ports'].forEach(i=>qs(i).value='');wizardStep(1);loadAll();showPage('tunnelsPage',document.querySelector('[data-page="tunnelsPage"]'))}}
