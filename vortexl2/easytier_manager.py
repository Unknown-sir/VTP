...
class EasyTierConfigManager:
    ...
    def next_defaults(self) -> Dict[str, Any]:
        used = self._used()
        index = 1
        while True:
            port = 2070 + index - 1
            ...
            iface = f"tun{index}"
            ip = f"10.155.{154 + index}.1"
            if port not in used["ports"] and rpc_port not in used["rpc_ports"] and iface not in used["interfaces"] and ip not in used["ips"]:
                return {"index": index, "port": port, "rpc_port": rpc_port, "interface_name": iface, "local_ip": ip}
            index += 1

    def create_tunnel(self, name: str) -> EasyTierConfig:
        safe = _safe_name(name)
        defaults = self.next_defaults()
