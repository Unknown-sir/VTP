...
GITHUB_REPO="${VTP_GITHUB_REPO:-Unknown-sir/VTP}"
GITHUB_BRANCH="${VTP_GITHUB_BRANCH:-main}"

# When install.sh is executed directly from GitHub with bash <(curl ...),
# the project files are not next to the script. In that case download the
# repository archive, then run this same installer from the extracted tree.
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
if [[ ! -d "$SCRIPT_DIR/vortexl2" ]]; then
  TMP_SRC="$(mktemp -d /tmp/vtp-install.XXXXXX)"
  ARCHIVE_URL="https://github.com/${GITHUB_REPO}/archive/refs/heads/${GITHUB_BRANCH}.tar.gz"
  if [[ -n "${VORTEXL2_GH_PROXY:-}" ]]; then
    ARCHIVE_URL="${VORTEXL2_GH_PROXY}${ARCHIVE_URL}"
  fi
  echo "[VTP] Downloading ${GITHUB_REPO}/${GITHUB_BRANCH}..."
  curl -fsSL "$ARCHIVE_URL" | tar -xz -C "$TMP_SRC" --strip-components=1
